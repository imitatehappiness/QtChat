...
void Client::setUsersRoom(int idRoom){
    QStringListModel* model = new QStringListModel(this);
    QStringList listUsername;

    QString query = "SELECT user_id FROM party WHERE chat_id = " + QString::number(idRoom);
    bool validate = mDB->setQuery(query);
    if(validate){
        QVector<QVector<QVariant>> usersID =  mDB->getData();
        for(int i = 0; i < usersID.size(); i++){
            QVector<QVector<QVariant>> username;
            query = "SELECT name FROM users WHERE id = " + QString::number(usersID[i][0].toInt()) + "";
            validate = mDB->setQuery(query);
            username = mDB->getData();
            listUsername.append(username[0][0].toString());
        }
    }

    model->setStringList(listUsername);
    ui->lV_usersRoom->setModel(model);
    ui->lV_usersRoom->setEditTriggers(QAbstractItemView::NoEditTriggers);
}
